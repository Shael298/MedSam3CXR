...
except Exception:  # pragma: no cover - platform dependent (e.g. Windows)
    triton = None
    tl = None
...
def edt_triton(data: torch.Tensor):
    ...
    # Fast path: Triton on CUDA (original behavior).
    if TRITON_AVAILABLE and data.is_cuda:
        B, H, W = data.shape
    ...
    # Fallback path: CPU OpenCV EDT.
    # This is slower, but it allows running on platforms without triton wheels (e.g. Windows).
    try:
        import numpy as np
        import cv2
    except Exception as exc:  # pragma: no cover
        raise ModuleNotFoundError(
            "EDT fallback requires opencv-python and numpy. Install MedSAM3 requirements."
        ) from exc

    device = data.device
    cpu = data.detach().to("cpu")
    masks = cpu.to(torch.uint8).numpy()
    B, H, W = masks.shape
    out = np.empty((B, H, W), dtype=np.float32)
    for b in range(B):
        out[b] = cv2.distanceTransform(masks[b], cv2.DIST_L2, 0)
